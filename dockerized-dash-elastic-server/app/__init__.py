...
if os.path.exists(file_path):
    print(
        "this is the first startup! I will upload the metrics of the initial model to elastic"
    )
    elastic_id = asyncio.run(load_initial_model_metrics_to_elastic())
    files = [
        APPPATH + MODELPATH + MODELNAME,
        APPPATH + MODELPATH + SCALERNAME,
        APPPATH + MODELPATH + IPCANAME,
    ]
...
def restore_model_to_previous_version(
    elastic_id: str, mc: Modelhash_Container
