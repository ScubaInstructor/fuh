...
def get_self_created_flow_data() -> DataFrame:
    ...
    if len(data) > 0:
        for_retraining = data[data["has_been_seen"] == True][
            ["flow_data", "attack_class"]
        ]
...
def merge_own_flows_into_trainigdataset_for_multiclassifier(own_data: DataFrame):
    ...
    trainingdata = load(
        "app/datasources/DataFrame_with_balanced_dataset.pkl"
    )  # load("DataFrame_with_balanced_dataset.pkl")
    if len(own_data) == 0:
        df = trainingdata
    else:
        ...
        for name in class_names:
            ...
            own_flows_of_same_class = own_data[
        ...
        remaining_classes = [
            cls for cls in all_classes if cls.lower() not in class_names
        ]
...
def create_boxplot_data_for_elastic(mergeddata):
    ...
    def _create_metrics_list(data):
        ...
        metrics.append(
            {
                "metric_name": "mean",
                **create_metric_data("mean", data.mean(numeric_only=True).to_frame().T),
            }
        )
        metrics.append(
            {
                "metric_name": "min",
                **create_metric_data("min", data.min(numeric_only=True).to_frame().T),
            }
        )
        metrics.append(
            {
                "metric_name": "max",
                **create_metric_data("max", data.max(numeric_only=True).to_frame().T),
            }
        )
        metrics.append(
            {
                "metric_name": "q1",
                **create_metric_data(
                    "q1", data.quantile(0.25, numeric_only=True).to_frame().T
                ),
            }
        )
        metrics.append(
            {
                "metric_name": "median",
                **create_metric_data(
                    "median", data.median(numeric_only=True).to_frame().T
                ),
            }
        )
        metrics.append(
            {
                "metric_name": "q3",
                **create_metric_data(
                    "q3", data.quantile(0.75, numeric_only=True).to_frame().T
                ),
            }
        )
...
def retrain() -> str:
    ...
    mergeddata = merge_own_flows_into_trainigdataset_for_multiclassifier(
        own_data=own_data
    )
    processed_data, scaler, ipca, ipca_size = adapt_cicids2017_for_training(
        data=mergeddata, balance_the_data=False
    )
    ...
    cec = CustomElasticsearchConnector()
    elastic_id = asyncio.run(
        cec.save_model_properties(
            hash_value=model_hash,
            timestamp=datetime.now(),
            own_flow_count=own_flow_count,
            score=score,
            confusion_matrix_data=cm_data_as_list,
            class_metric_data=metrics,
