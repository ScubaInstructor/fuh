...
try:
    ...
    if df.empty:
        ...
        min_flow_data = (
            flow_data.select_dtypes(include="number")
            .drop(["src_port", "dst_port", "protocol"], axis=1)
            .min()
        )
        max_flow_data = (
            flow_data.select_dtypes(include="number")
        ...
        min_flow_data = pd.DataFrame(
            [min_flow_data], columns=mean_flow_data.index.to_list()
        )
        max_flow_data = pd.DataFrame(
            [max_flow_data], columns=mean_flow_data.index.to_list()
        )
        mean_flow_data = pd.DataFrame(
            [mean_flow_data], columns=mean_flow_data.index.to_list()
        )
...
def make_modal():
    return dbc.Modal(
        [
            ...
            dbc.ModalBody(
                "This is the content of the modal", id="classified-modal-body"
            ),
            dbc.ModalFooter(
                [
                    dbc.Button(
...
@callback(
    Output("classified-modal-body", "children"), Input("seen_grid", "selectedRows")
)
def update_modal_content(selected_rows):
    ...
    return [
        dcc.Store(
            id="classified-selected-row-store", data=detail_df.to_dict("records")
        ),
        ...
        dbc.Accordion(
            [
                dbc.AccordionItem(
                    children=[
                        html.Div(id="classified-boxplot-content")
            ...
            id="classified-accordion",
        ),
        dbc.Row(
            [
                html.H5(
                    children="Selected Attack Type Classification",
                    className="mb-0 mb-1",
                ),
                dcc.Dropdown(
...
def update_grid(
    ...
    n_clicks_reset,
    clickData_map,
    df_with_location_classified_data,
    selected_type_value,
    selected_row_data,
):
    ...
    if trigger == "classified-reset-grid" and n_clicks_reset:
        df_update = asyncio.run(
            cec.get_all_flows(view="seen", size=flow_nr, include_pcap=False)
        )
        seen_data = df_update[df_update["has_been_seen"] == True].to_dict("records")
        return (
            seen_data,
            create_world_map("world-map-classified", pd.DataFrame(seen_data)).figure,
        )
    ...
    if trigger == "classified-submit-classification" and n_clicks_submit:
        ...
        try:
            asyncio.run(
                cec.set_attack_class(flow_id=flow_id, attack_class=selected_type_value)
            )
...
@callback(Output("retrain-button", "children"), Input("url", "pathname"))
...
@callback(
    [Output("retrain-status", "children"), Output("retrain-model", "disabled")],
...
def retrain_model(n_clicks):
    ...
    try:
        mc.set_hash(retrain())
        return (
            html.Div("Model retrained successfully!", style={"color": "green"}),
            False,
        )
    except Exception as e:
        return (
            html.Div(f"Error retraining model: {str(e)}", style={"color": "red"}),
            False,
        )
        ...
        try:
            ...
            if df.empty:
                ...
                df["source_lat"], df["source_lon"] = zip(
                    *df["partner_ip"].apply(ip_to_lat_lon)
                )
