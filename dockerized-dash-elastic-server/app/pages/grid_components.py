...
def make_detailed_grid(grid_id, selected_rows):
    ...
    detail_grid = dag.AgGrid(
        ...
        columnDefs=[
            ...
            if (col != "flow_data" and col != "probabilities")
        ],
        style={"height": 200},
    )
    return detail_grid
...
def create_boxplot(detail_flow_df, prediction):
    ...
    model_df = pd.DataFrame(pd.DataFrame(x.iloc[0]["boxplotdata"][0]["metrics"])).drop(["metric_name","icmp_type","icmp_code"], axis=1)
    model_df = model_df.apply(lambda x: x.str[0])

    model_pred_df = pd.DataFrame(pd.DataFrame(x.iloc[0]["boxplotdata"][boxplot_index]["metrics"])).drop(["metric_name","icmp_type","icmp_code"], axis=1)
    ...
    for column in norm_model_df.columns:
        ...
        scatter_trace = go.Scatter(
            ...
            marker=dict(
                color="red", size=8, opacity=0.6, symbol="cross"
    ...
    fig.update_layout(
        ...
        xaxis=dict(title="Normalized Values", gridcolor="lightgray", showgrid=True),
        yaxis=dict(
            title="Columns",
            type="category",
        ...
        height=len(norm_model_df.columns) * 40
        + 200,  # Adjust this value to control overall height
