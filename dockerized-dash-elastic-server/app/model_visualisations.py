...
class Model_Visualisator:
    ...
    def create_storeable_list_from_cunfusion_matrix(
        self, confusion_matrix_data: ndarray, classes: list
    ) -> list[dict]:
        ...
        for i, row in enumerate(confusion_matrix_data):
            for j, value in enumerate(row):
                data_to_index.append(
                    {"actual": classes[i], "predicted": classes[j], "value": int(value)}
                )
    ...
    def create_confusion_matrix_figure_from_elastic_data(self, data: list[dict]):
        ...
        df = DataFrame(data)
        confusion_matrix_data = df.pivot_table(
            index="actual", columns="predicted", values="value"
        ).fillna(0)
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                autorange="reversed"
            ),  # To display the y-axis in the correct order
            margin=dict(l=100, r=100, b=100, t=100, pad=4),
        )

        return fig

    def create_metrics_overview_from_elastic_data(
        self, class_metric_data: list[list], classes: list
    ):
    ...
    def create_boxplot_for_all_models(self, df: DataFrame) -> go.Figure:
        ...
        labels = [
            datetime.strftime(d, "%Y-%m-%d %H:%M") for d in df["timestamp"].tolist()
        ]
        ...
        fig = go.Figure(
            go.Bar(
                ...
                orientation="h",
                marker=dict(
                    color=scores, colorscale="Greens"
...
if __name__ == "__main__":
    ...
    metrics_fig = mv.create_metrics_overview_from_elastic_data(
        retrieved_metrics, classes=classes
    )
    metrics_fig.show()
