...
def notify_users():
    ...
    if last_notification < datetime.now() - timedelta(
        minutes=DISCORD_NOTIFICATION_DELAY
    ):
...
def upload():
    ...
    try:
        ...
        if (
            ...
            and "probabilities" in request.json
            and "timestamp" in request.json
            and "prediction" in request.json
            and "sensor_name" in request.json
            and "sensor_port" in request.json
            and "partner_ip" in request.json
            and "partner_port" in request.json
            and "has_been_seen" in request.json
            and "attack_class" in request.json
            and "flow_id" in request.json
            and "model_hash" in request.json
        ):
...
def get_latest_model():
    ...
    try:
        ...
        if Path("app/" + filename).is_file():
            return send_file(filename, as_attachment=True, download_name=ZIPFILENAME)
