...
class My_Sniffer:
    def __init__(self) -> None:
        ...
        self.snif: AsyncSniffer = (
            self.get_intern_sniffer()
    ...
    def start(self):
        ...
        Thread(
            target=self.check_for_model_update(), daemon=True, name="model_updater"
        ).start()
        worker_thread = (
            self.start_receiver_worker()
        )  # Start worker to deal with items in the queue
    ...
    def get_intern_sniffer(self) -> AsyncSniffer:
        ...
        s: AsyncSniffer = sniffer.create_sniffer(
            input_file=None,
            input_interface=SNIFFING_INTERFACE,
            output_mode="intern",
    ...
    def worker(self):
        ...
        Returns: None this means the model needs to be updated
        ...
        while True:
            ...
            flow_data: DataFrame = adapt_for_prediction(
                data=flow_data, scaler=self.scaler, ipca=self.ipca, ipca_size=IPCASIZE
            )
            self.model_lock.acquire()
            ...
            if prediction != ["BENIGN"] or proba.max() < 0.8 or DEBUGGING:
                ...
                flow_id = str(uuid4())
                # Create a PCAP file
                flow_bytesIO = create_BytesIO_pcap_file(
                    item
                )  # the pcap file as BytesIO object  DEPRECATED
                ...
                if DEBUGGING:
                    print(
                        f"my_ip: {my_ip} {type(my_ip)}, dest_ip: {item.dest_ip} {type(item.dest_ip)}"
                    )
    ...
    def download_new_model(self):
        """
        Download the new model, scaler and ipca, replacing the old files.
        """
